...
public class DispatchServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            // 현재 각 pageController에 해당하는 Servlet클래스들은 request공간에서 Parameter를 꺼내도록 구현되어 있다.
            // 하지만 Servlet의 종속성을 약화하고 보다 독립성을 강화하기 위해 DispatchServlet을 제외한 나머지 Servlet 클래스들은
            // HttpServlet을 상속받지 않은 Pojo방식의 java클래스로 변경할 예정이다.
            // 그렇게 되면, request공유 공간을 사용할 수 없으므로 이곳에서는 DispatchServlet에서는 먼저 Parameter를 꺼내서 처리하도록 하고
            // 나중에는 다른 방식으로 각 pageController에 전달할 예정이다.
            if ("/memberLinkQuestion/list.do".equals(servletPath)){
                pageControllerPath = "/memberLinkQuestion/list";
            } else if("/memberLinkQuestion/add.do".equals(servletPath)){
                pageControllerPath = "/memberLinkQuestion/add";
            ...
            } else if("/memberLinkQuestion/update.do".equals(servletPath)){
                pageControllerPath = "/memberLinkQuestion/update";
                if(req.getParameter("email") != null){
                    req.setAttribute("member", new Member()
            ...
            } else if("/memberLinkQuestion/delete.do".equals(servletPath)){
                pageControllerPath = "/memberLinkQuestion/delete";
            } else if("/authLinkQuestion/login.do".equals(servletPath)){
                pageControllerPath = "/authLinkQuestion/login";
            } else if("/authLinkQuestion/logout.do".equals(servletPath)){
                pageControllerPath = "/authLinkQuestion/logout";
            }
            ...
            if(viewUrl.startsWith("redirect:")){
                // viewUrl에서 'redirect:' 글자를 제외한 나머지 url를 추출해서 브라우저에 redirect 요청을 보낸다.
                // 브라우저는 redirect 요청을 받으면 해당 url로 다시 접속한다.
